...
<body class="w-screen h-full m-0 p-0 md:flex md:flex-row">
    ...
    <main class="w-full h-screen m-0 p-0 bg-red-400 bg-[url('img/fondo2.jpg')] bg-cover bg-no-repeat bg-local overflow-y-scroll flex items-center flex-col">
            ...
            <div class="popup-inner bg-[rgba(29, 29, 27, 0.9);] rounded-lg shadow-md p-6 max-w-md">
                ...
                <form id="commentForm" class="comment-form flex flex-col" onsubmit="enviarComentario(event)">
                    ...
                    if ($username === "Undefined") {
                        // Mostrar un mensaje indicando que el usuario necesita estar logueado
                        echo '<div class="text-red-500 mb-4">Por favor, inicia sesión para poder comentar.</div>';
                        echo '<button type="button" class="bg-red-500 text-gray-900 px-4 py-2 rounded-md hover:bg-red-400 transition-colors focus:outline-none" onclick="cerrarPopup()">Salir</button>';
                    } else {
                    ...
                    <div class="mb-4">
                        ...
                        <textarea id="commentContent" name="commentContent" required="required" class="px-4 py-2 border border-gray-300 rounded-md mt-1 focus:outline-none focus:ring-yellow-500 focus:border-yellow-500" rows="4" placeholder="Escribe tu comentario aquí"></textarea>
                    </div>
        ...
        <section class="w-full h-full flex flex-col items-center bg-[url('img/trophie.jpg')] bg-cover bg-no-repeat bg-local bg-opacity-10 md:w-10/12 overflow-hidden overflow-y-auto border-sky-200 shadow-[0_0_2px_#fff,inset_0_0_2px_#fff,0_0_5px_#ff0,0_0_15px_#ff0,0_0_30px_#ff0] scroll-bonito">
